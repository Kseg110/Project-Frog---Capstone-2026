...
public class PoisonBomb : MonoBehaviour
{
    ...
    void Explode()
    {
        ...
        Collider[] hits = Physics.OverlapSphere(transform.position, explosionRadius, affectLayerMask, QueryTriggerInteraction.Ignore);
        ...
        foreach (var hit in hits)
        {
            ...
            // choose root object so statuses survive if hitbox is a child
    ...
    bool IsValidTarget(Collider hit)
    {
        ...
        // Player
        if ((targetMode == TargetMode.Player || targetMode == TargetMode.Both) && hit.gameObject.CompareTag("Player"))
            return true;

        // Enemy
        if ((targetMode == TargetMode.Enemy || targetMode == TargetMode.Both))
        {
            if (hit.gameObject.CompareTag("Enemy")) return true;
            if (hit.GetComponentInParent<EnemyBase>() != null) return true;
            if (hit.GetComponentInParent<Health>() != null && !hit.gameObject.CompareTag("Player")) return true;
        }
...
public class PoisonedStatus : MonoBehaviour
{
    ...
    void ApplyDamage()
    {
        ...
        // Player or generic health fallback
